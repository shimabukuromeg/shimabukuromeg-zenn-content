...
- [Create Figma Plugin](https://github.com/yuanqing/create-figma-plugin) の存在を知り、使ってみたかった。

この記事が、Create Figma Pluginを使ってFigma Pluginの開発を始めたい方にとって参考になれば嬉しいです。

# Figma Pluginとは

Figmaでは、プラグインをインストールして機能を拡張できます。詳細は [公式ドキュメント](https://www.figma.com/plugin-docs/)をご覧ください。
...
さらに、Figma Pluginは自作も可能です。公式[セットアップガイド](https://www.figma.com/plugin-docs/setup/)を参考にすれば、簡単なサンプルプラグイン（四角形を生成するプラグイン）を試せます。
